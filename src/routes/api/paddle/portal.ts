...
export const Route = createFileRoute("/api/paddle/portal")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        if (!PADDLE_API_KEY) {
          return new Response(JSON.stringify({ error: "Paddle API key not configured" }), {
            status: 500,
        ...
        if (subError || !sub?.paddle_customer_id) {
          return new Response(
            JSON.stringify({
              ...
              message: "No Paddle customer found for this account.",
            }),
        ...
        if (!portalUrl) {
          ...
          return new Response(
            JSON.stringify({
              error: "no_portal_url",
              message: "Portal session created but no URL returned.",
            }),
            { status: 502, headers: { "content-type": "application/json" } },
          );
        }
